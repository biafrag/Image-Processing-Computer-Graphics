...
void smoothing(Image& input, Image& output) //usar filtro gaussiano -> ok
{
    ...
    int height,width,depth,i,j,posmed;
    std::vector<int> Vpos;
    Vpos.resize(9);
...
void haar(Image& input, Image& output) //ok
{
    ...
    for(n=0;n<depth;n++)
    {
        ...
        {
            for(i=0,l=0;i<(maxwidth);i=i+2,l++)
            {
        ...
        for(i=0;i<width;i++)
        {
            for(j=0,l=0;j<maxheight;j=j+2,l++)
            {
               pos=j*width*depth+i*depth+n;
...
void haarInv(Image& input, Image& output) //ok
{
    int i,n,j,posf,pos,pos1,l;
    int height,width,depth;
    std::vector<float> in=input.getColorBuffer();
